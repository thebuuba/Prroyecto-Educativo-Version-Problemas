import{t as u,S as p,i as v,a as y,k as S,l as E,a5 as x,a7 as m}from"./main-BqMwzFXj.js";function $({FormState:d,updateSync:i}){window.updateStudentEditField=(s,t)=>{if(s==="matricula"){t=t.replace(/\D/g,"").slice(0,9),t.length>2&&(t=`${t.slice(0,2)}-${t.slice(2)}`),t.length>7&&(t=`${t.slice(0,7)}-${t.slice(7)}`);const o=document.getElementById("se-mat");o&&(o.value=t)}d[s]=t,i()},window.handleStudentEditPhoto=s=>{var n;const t=(n=s==null?void 0:s.files)==null?void 0:n[0];if(!t)return;const o=new FileReader;o.onload=a=>{d.photoUrl=a.target.result,i()},o.readAsDataURL(t)},window.confirmSaveEditStudent=async()=>{if(typeof window.saveEditStudent!="function"){u("Error: Motor de edición no disponible",!0);return}try{await window.saveEditStudent()}catch(s){console.error("[EduGest][student-edit] Error:",s),u("Error al actualizar estudiante",!0)}},window.handleDeleteStudent=s=>{typeof window.delEst=="function"?window.delEst(s):u("Motor de eliminación no disponible",!0)}}let l=null;const e={nombre:"",apellido:"",matricula:"",courseId:"",photoUrl:""};function k(d){const i=p.editingStudentId;if(l=p.estudiantes.find(a=>a.id===i),!l){d.innerHTML=`
    ...
    `;return}e.nombre=l.nombre,e.apellido=l.apellido,e.matricula=l.matricula,e.courseId=l.courseId||l.sectionId||l.seccionId,e.photoUrl=l.photoUrl||"";const s=v(y()),t=S(l.id,e.courseId),{l:o,c:n}=t!==null?E(t):{l:"?",c:"bg-slate-100 text-slate-400"};d.innerHTML=`
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center gap-8 w-full">
           ...
           <div class="relative group">
              <div class="w-40 h-40 rounded-[2.5rem] overflow-hidden bg-white/10 border-4 border-white shadow-xl">
                 <img id="se-photo-preview" src="${e.photoUrl||x(`${e.nombre} ${e.apellido}`)}" class="w-full h-full object-cover">
              </div>
           ...
           <div class="flex-1 text-center md:text-left">
              ...
              <h1 class="text-4xl md:text-5xl font-black tracking-tight" id="se-header-name">${e.nombre} ${e.apellido}</h1>
              <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-4">
                 <div class="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl text-sm font-medium border border-white/5">
                    ...
                    <span id="se-header-mat">${m(e.matricula)}</span>
                 </div>
                 <div class="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl text-sm font-medium border border-white/5">
                    <span class="material-symbols-outlined text-sm text-slate-400">school</span>
                    <span id="se-header-grade">${f(e.courseId)}</span>
                 </div>
              </div>
           </div>
           ...
           <div class="flex flex-col items-center md:items-end gap-3">
              ...
              <div class="w-20 h-20 rounded-3xl ${n.replace("bg-","bg-opacity-20 ")} bg-blue-500/20 flex flex-col items-center justify-center border-2 border-white/10">
                 <div class="text-2xl font-black text-white">${t!==null?t:"—"}</div>
                 <div class="text-[10px] font-black text-blue-400">${o}</div>
              </div>
           </div>
        </div>
        ...
        <div class="lg:col-span-8 space-y-8">
          ...
          <section class="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                ...
                <input type="text" id="se-nom" value="${e.nombre}"
              ...
              <div class="space-y-2">
                ...
                <input type="text" id="se-ape" value="${e.apellido}"
                       class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-medium text-slate-800 focus:ring-4 focus:ring-blue-50 outline-none transition-all"
              ...
              <div class="space-y-2">
                ...
                <input type="text" id="se-mat" value="${m(e.matricula)}"
                       class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-medium text-slate-800 focus:ring-4 focus:ring-blue-50 outline-none transition-all"
          ...
          <section class="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ${s.map(a=>{const r=e.courseId===a.id;return`
                  <button 
                    onclick="window.updateStudentEditField('courseId', '${a.id}')"
                  ...
                  >
                    <div class="font-black text-slate-900 leading-tight">${a.grado} ${a.sec}</div>
                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-1 truncate">${a.materia}</div>
  ...
  `,g()}function f(d){const i=p.secciones.find(s=>s.id===d);return i?`${i.grado} ${i.sec}`:"Sin Asignar"}function g(){const d=document.getElementById("se-header-name"),i=document.getElementById("se-header-mat"),s=document.getElementById("se-header-grade"),t=document.getElementById("se-photo-preview"),o=document.getElementById("se-save-btn"),n=`${e.nombre} ${e.apellido}`;d&&(d.textContent=n||"Sin Nombre"),i&&(i.textContent=m(e.matricula)||"—"),s&&(s.textContent=f(e.courseId));const a=x(n);t&&(t.src=e.photoUrl||a);const r=e.nombre&&e.apellido&&e.matricula&&e.courseId;o&&(o.disabled=!r);const c=(h,w)=>{const b=document.getElementById(h);b&&(b.value=w)};c("ee-nom",e.nombre),c("ee-ape",e.apellido),c("ee-mat",e.matricula),c("ee-sec",e.courseId),c("ee-photo-data",e.photoUrl)}function A(){$({FormState:e,updateSync:g}),window.RENDERS||(window.RENDERS={}),window.RENDERS["student-edit"]=k}export{A as init,k as renderStudentEditPanel};
