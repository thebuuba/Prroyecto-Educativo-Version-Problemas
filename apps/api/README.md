...
- `GET /health`, `GET /api/users`, `POST /api/users`, `GET /api/schools` y `POST /api/schools` ya siguen el patrón documentado en `../../docs/BACKEND_LAYERING_PATTERN.md`.
- Para conectar Supabase Postgres, configura `DATABASE_URL` con el connection string del proyecto y `DATABASE_SSL=true`.
- El Postgres local de `docker-compose.yml` publica `localhost:5433` para evitar conflictos con instalaciones locales en `5432`.
...
- `npm run migrate` aplica `../../supabase/schema.sql` y las migraciones idempotentes de `../../supabase/migrations` contra la base configurada en `DATABASE_URL`.
