import{M as u,N as p,O as d,P as b,Q as x,R as v,T as g,U as f,V as m,W as h,X as c}from"./main-BFkCmjLr.js";function r(){return b(),window.S}function w(){const e=r();e.lessonPlanDraft=u(e.activeGroupId,e.activePeriodId),e.lessonPlanUi.mode="editor",e.lessonPlanUi.activeSectionId="general",window.go("planificaciones")}function y(e){const s=r(),l=s.lessonPlans.find(t=>t.id===e);l&&(s.lessonPlanDraft=JSON.parse(JSON.stringify(l)),s.lessonPlanUi.mode="editor",s.lessonPlanUi.activeSectionId=l.editorStep||"general",window.go("planificaciones"))}function k(){const e=r();e.lessonPlanUi.mode="home",window.go("planificaciones")}function P(e){const s=r();s.lessonPlanUi.activeSectionId=e,p(),window.go("planificaciones")}function $(e,s){const l=d(),t=e.split(".");t.length===2?l[t[0]][t[1]]=s:l[e]=s}function C(e,s){const l=d();l.curriculum[e]=s}function S(){window.lessonPlanNew=w,window.lessonPlanContinue=y,window.lessonPlanReturnHome=k,window.lessonPlanSetActiveSection=P,window.lessonPlanSetGeneralField=$,window.lessonPlanSetCurriculumField=C}function j(e,s){return`
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      ...
      <div class="flex items-center gap-3">
        <div class="px-4 py-2 bg-white rounded-2xl border border-slate-200 shadow-sm">
          ...
          <span class="text-lg font-bold text-slate-700">${s}</span>
        </div>
        <div class="px-4 py-2 bg-white rounded-2xl border border-slate-200 shadow-sm">
  ...
  `}function E(e){return e.length?`
    <div class="space-y-3">
      ${e.slice(0,3).map(s=>{var l,t;return`
        <div onclick="lessonPlanContinue('${s.id}')" class="flex items-center gap-4 p-4 bg-white border border-slate-100 rounded-2xl hover:border-blue-200 hover:bg-blue-50/30 transition-all cursor-pointer group">
          ...
          <div class="min-w-0 flex-1">
            <h4 class="font-bold text-slate-800 truncate">${((l=s.general)==null?void 0:l.themeTitle)||((t=s.general)==null?void 0:t.title)||"Sin título"}</h4>
            <p class="text-xs text-slate-500">${g(s.updatedAt)}</p>
          </div>
  ...
  `}function M(e){const s=f(),l=m(e.general.gradeId);return`
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="flex flex-col gap-1.5 col-span-2">
              ...
              <select onchange="lessonPlanSetGeneralField('general.gradeId', this.value)" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none text-slate-700 font-medium appearance-none cursor-pointer">
                ...
                ${s.map(t=>`<option value="${t.id}" ${e.general.gradeId===t.id?"selected":""}>${t.name}</option>`).join("")}
              </select>
            </div>
            <div class="flex flex-col gap-1.5">
              ...
              <select onchange="lessonPlanSetGeneralField('general.sectionName', this.value)" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none text-slate-700 font-medium appearance-none cursor-pointer">
                ...
                ${l.map(t=>`<option value="${t.value}" ${e.general.sectionName===t.value?"selected":""}>${t.label}</option>`).join("")}
              </select>
            </div>
            <div class="flex flex-col gap-1.5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <select onchange="lessonPlanSetGeneralField('general.transversalAxis', this.value)" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none text-slate-700 font-medium appearance-none cursor-pointer">
              ...
              ${h.map(t=>`<option value="${t.value}" ${e.general.transversalAxis===t.value?"selected":""}>${t.value}</option>`).join("")}
            </select>
          </div>
        </div>
      </div>
  ...
  `}function A(e){return`
    <div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div class="space-y-6">
        ${e.classes.map((s,l)=>{var t,a,n;return`
          <div class="bg-slate-50 border border-slate-200 rounded-3xl overflow-hidden group">
            <div class="p-6 flex items-center justify-between bg-white border-b border-slate-100">
              <div class="flex items-center gap-4">
                ...
                <div>
                  <h5 class="font-bold text-slate-800">${s.title||`Clase ${l+1}`}</h5>
                  <p class="text-xs text-slate-500 font-medium uppercase tracking-wider">${c(s.date)||"Sin fecha"}</p>
                </div>
              </div>
            ...
            <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
              <div class="space-y-3">
                ...
                <textarea rows="4" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all text-sm leading-relaxed" placeholder="Introducción, saberes previos...">${((t=s.start)==null?void 0:t.description)||""}</textarea>
              </div>
              <div class="space-y-3">
                <label class="text-xs font-bold text-slate-400 uppercase tracking-wider">Desarrollo (70%)</label>
                <textarea rows="4" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all text-sm leading-relaxed" placeholder="Actividades centrales...">${((a=s.development)==null?void 0:a.description)||""}</textarea>
              </div>
              <div class="space-y-3">
                <label class="text-xs font-bold text-slate-400 uppercase tracking-wider">Cierre (15%)</label>
                <textarea rows="4" class="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/20 transition-all text-sm leading-relaxed" placeholder="Síntesis y evaluación...">${((n=s.closure)==null?void 0:n.summary)||""}</textarea>
              </div>
            </div>
          </div>
        `}).join("")}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div class="space-y-6">
          <div class="space-y-3">
            ...
            <textarea oninput="lessonPlanSetGeneralField('strategy.methodology', this.value)" rows="4" class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none text-slate-700 font-medium leading-relaxed">${((s=e.strategy)==null?void 0:s.methodology)||""}</textarea>
          </div>
          <div class="space-y-3">
    ...
    <div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div class="bg-white border border-slate-200 shadow-sm rounded-3xl overflow-hidden text-slate-800 font-serif p-10 max-w-[800px] mx-auto text-left">
        ...
        <div class="grid grid-cols-2 gap-y-3 text-sm mb-10">
          ...
          <div class="flex gap-2"><span class="font-bold">Fecha:</span> <span>${c(e.general.startDate)||"---"}</span></div>
            ...
            <div class="space-y-6">
              ${e.classes.map((s,l)=>{var t,a,n;return`
                <div class="p-4 border border-slate-100 rounded-2xl">
                  <span class="text-xs font-bold text-slate-400">CLASE ${l+1}: ${s.title||"Tema diario"}</span>
                  <div class="grid grid-cols-1 gap-2 mt-2 text-sm">
                    <p><span class="font-bold italic">Inicio:</span> ${((t=s.start)==null?void 0:t.description)||"---"}</p>
                    <p><span class="font-bold italic">Desarrollo:</span> ${((a=s.development)==null?void 0:a.description)||"---"}</p>
                    <p><span class="font-bold italic">Cierre:</span> ${((n=s.closure)==null?void 0:n.summary)||"---"}</p>
                  </div>
                </div>
              `}).join("")}
            </div>
  ...
  `}function G(e){const s=r(),l=s.lessonPlanUi.mode||"home",t=x(),a=v(s.activeGroupId),n=a.length;if(l==="home"){e.innerHTML=`
    ...
    `;return}const i=s.lessonPlanDraft,o=s.lessonPlanUi.activeSectionId||"general";e.innerHTML=`
  ...
  `}function B(){S(),window.RENDERS||(window.RENDERS={}),window.RENDERS.planificaciones=G}export{B as inicializar};
