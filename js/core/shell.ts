 ...
 */
export function updateSBUser(): void {
  ...
  const logoutBtns = document.querySelectorAll<HTMLElement>('[data-shell-action="logout"], .sb-logout-btn, #sb-logout');
 ...
 */
export function syncSidebarNavState(activePage: string = S.currentPage || 'tablero'): void {
 ...
 */
export function closeProfileMenu(): void {
...
export function initShell(): void {
  ...
  document.addEventListener('click', (event) => {
    ...
    const logoutButton = event.target?.closest?.('[data-shell-action="logout"], .sb-logout-btn, #sb-logout, button[onclick="logoutAuth()"]');
    if (logoutButton) {
      event.preventDefault();
      event.stopPropagation();
 ...
 */
export function initAICopilot(): void {
  ...
  const syncAICopilotVisibility = (): boolean => {
    const authModal = document.getElementById('m-auth');
  ...
  const appendMessage = (text: string, type: ChatRole, id: string | null = null): void => {
