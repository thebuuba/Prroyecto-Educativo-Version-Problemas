(function initEduGestCloud(global) {
  ...
  // Obtiene get config.
  function getConfig() {
    return global.EDUGEST_FIREBASE_CONFIG || null;
  }
...
'auth/user-disabled': 'Tu cuenta esta deshabilitada temporalmente.',
  ...
  async function buildDeviceContext() {
    const deviceId = readOrCreateStorageValue(global.localStorage, DEVICE_ID_STORAGE_KEY, 'dv_');
    const sessionId = readOrCreateStorageValue(global.sessionStorage, DEVICE_SESSION_STORAGE_KEY, 'ss_');
    const userAgent = String(global.navigator?.userAgent || '');
    const language = String(global.navigator?.language || 'es-DO');
    ...
    return {
      ...
      platform: String(global.navigator?.platform || ''),
    };
  }
  ...
  async function saveUserProfile(user, extraProfile = {}) {
    return true;
  }
  ...
  async function inspectEmailAccount(email) {
  ...
  async function startDeviceVerification(options = {}) {
    void options;
    return null;
  }

  // Gestiona verify device verification code.
  async function verifyDeviceVerificationCode(code = '') {
    void code;
    return true;
  }

  // Gestiona lista account security.
  async function listAccountSecurity() {
    return null;
  }

  // Gestiona revoke trusted device.
  async function revokeTrustedDevice(deviceIdHash = '') {
    void deviceIdHash;
    return null;
  }
  ...
  async function reportSuspiciousSignal(payload = {}) {
    void payload;
    return null;
  }

  // Crea crear paddle checkout.
  async function createPaddleCheckout(priceId, options = {}) {
    void priceId;
    void options;
    return null;
  }
  ...
  async function createPaddleCustomerPortal(returnUrl = '') {
    void returnUrl;
    return null;
  }
  ...
  global.EduGestCloud = {
    ...
    createPaddleCustomerPortal,
    buildDeviceContext,
    listAccountSecurity,
    registerDeviceSession,
    reportSuspiciousSignal,
    revokeTrustedDevice,
    saveUserProfile,
