...
import { registerStudentEditActions } from './student-edit-actions.js';
 ...
 */
export function renderStudentEditPanel(container) {
           ...
           <div class="flex-1 text-center md:text-left">
              ...
              <div class="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-4">
                 ...
                 <div class="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl text-sm font-medium border border-white/5">
                    ...
                    <span id="se-header-grade">${getSectionLabel(FormState.courseId)}</span>
                 </div>
              </div>
           </div>
 ...
 */
function getSectionLabel(secId) {
...
function updateSync() {
  ...
  if (hGr) hGr.textContent = getSectionLabel(FormState.courseId);
...
export function init() {
  registerStudentEditActions({ FormState, updateSync });
  if (!window.RENDERS) window.RENDERS = {};
  window.RENDERS['student-edit'] = renderStudentEditPanel;
}
