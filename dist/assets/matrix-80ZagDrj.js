import{s as k,S as p,C as v,g as $,e as c,f as g,D as y,h as E,B as h,j as C,p as z,c as A,d as j}from"./main-Dd9jDrfc.js";function f(){return`
    <header class="mb-10 animate-in fade-in slide-in-from-top-4 duration-500">
      <div class="flex items-center justify-between gap-6">
        ...
        <div class="hidden md:flex flex-col items-end">
           <div class="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">${C(p.activeGroupId)}</div>
           <div class="text-sm font-bold text-indigo-600">${z()}</div>
        </div>
      </div>
    </header>
  `}function B(d){return`
    <div class="py-20 text-center bg-white border border-slate-200 rounded-[2.5rem] shadow-sm animate-in zoom-in-95 duration-500">
      ...
      <div class="flex items-center justify-center gap-4">
        ${d.length===0?`<button onclick="window.go('estudiantes')" class="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">Registrar Estudiantes</button>`:""}
  ...
  `}function M(d){const x=k(p.activeGroupId);if(x.length===0||v()===0){d.innerHTML=`
      <div class="max-w-[1240px] mx-auto p-6 md:p-10">
        ${f()}
        ${B(x)}
      </div>
    `;return}const m=$(p.activeGroupId),a={};c.forEach(t=>a[t]=m[t].activities);let o=`
  ...
  `;c.forEach(t=>{const e=a[t].length;e>0&&(o+=`
  ...
  `,c.forEach(t=>{a[t].forEach((e,b)=>{o+=`
        <th class="p-3 text-center min-w-[100px] border-r border-slate-200/40">
          ...
          <div class="text-[9px] font-medium text-slate-400 mt-0.5">/${g(e.pts)}pt</div>
        </th>
      `}),a[t].length>0&&(o+='<th class="p-3 text-center bg-white/50 border-r border-slate-200 font-black text-[11px] text-slate-900">Total B</th>')}),o+=`
  ...
  `;let r='<tbody class="bg-white divide-y divide-slate-50">';x.forEach(t=>{r+=`
    ...
    `;const e=[];c.forEach(l=>{if(a[l].forEach(s=>{const i=(p.notas[t.id]||{})[s.id],n=i===0||!!i,u=n?y(i,s.pts):"",w=u.includes("p-green")?"bg-emerald-500":u.includes("p-amber")?"bg-amber-500":u.includes("p-rose")?"bg-rose-500":"bg-slate-200";r+=`
          <td class="p-3 text-center cursor-not-allowed">
            <div class="relative inline-block px-2 py-1 rounded-lg ${n?"bg-slate-50 font-bold text-slate-900":"text-slate-300"}" title="Evaluado vía Panel de Actividades">
               ${n?g(i):'<span class="opacity-30">---</span>'}
               ${n?`<div class="absolute -top-1 -right-1 w-2 h-2 rounded-full border-2 border-white ${w}"></div>`:""}
            </div>
          </td>
        `}),a[l].length>0){const s=E(t.id,l);e.push(typeof s=="number"?s:0),r+=`<td class="p-3 text-center bg-slate-50/50 font-black text-sm text-slate-900 border-r border-slate-200/60" style="color: ${h[l]}">${s}</td>`}});const b=e.length?Math.round(e.reduce((l,s)=>l+s,0)/e.length):0;r+=`
  ...
  `}window.RENDERS.matriz=M;export{M as renderMatrixPanel};
