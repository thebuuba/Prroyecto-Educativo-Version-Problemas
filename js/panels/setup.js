 ...
 */
export function handlePhoneInput(el) {
  if (!el) return;
 ...
 */
export function updateInstitutionInlineHint(inputEl = null) {
...
export function handleInstitutionInput(inputEl) {
  if (!inputEl) return;
  inputEl.value = String(inputEl.value || '').replace(/\s{2,}/g, ' ');
  updateInstitutionInlineHint(inputEl);
}

export function handleInstitutionKeydown(event) {
