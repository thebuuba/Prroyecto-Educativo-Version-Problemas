import{i as w,S as x,x as m,c as k,B as n,o as f,y as $,z as E,q as u,r as y,v as A}from"./main-DiLCuoHx.js";function S(h){const c=w(x.activeGroupId);if(c.length===0||m()===0){h.innerHTML=`
  ...
  `;n.forEach(t=>{const e=a[t].length;e>0&&(r+=`
        <th colspan="${e+1}" class="p-5 text-center border-r border-slate-200/50" style="background: ${u[t]}10; color: ${u[t]}">
  ...
  `;let l='<tbody class="bg-white divide-y divide-slate-50">';c.forEach(t=>{l+=`
    ...
    `;const e=[];n.forEach(o=>{if(a[o].forEach(s=>{const d=(x.notas[t.id]||{})[s.id],i=d===0||!!d,b=i?$(d,s.pts):"",v=b.includes("p-green")?"bg-emerald-500":b.includes("p-amber")?"bg-amber-500":b.includes("p-rose")?"bg-rose-500":"bg-slate-200";l+=`
        ...
        `}),a[o].length>0){const s=E(t.id,o);e.push(typeof s=="number"?s:0),l+=`<td class="p-3 text-center bg-slate-50/50 font-black text-sm text-slate-900 border-r border-slate-200/60" style="color: ${u[o]}">${s}</td>`}});const p=e.length?Math.round(e.reduce((o,s)=>o+s,0)/e.length):0;l+=`
  ...
  `}function B(){window.RENDERS||(window.RENDERS={}),window.RENDERS.matriz=renderMatrixPanel}export{B as inicializar,S as renderizarMatrixPanel};
