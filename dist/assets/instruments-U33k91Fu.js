import{S as s,n as d,t as n,p as x,y as r,k as v,z as f}from"./main-Dd9jDrfc.js";const l=["rubrica_analitica","lista_cotejo_a","lista_cotejo_b","escala_estimativa"],a={rubrica_analitica:{title:"Rúbrica Analítica",icon:"📊",desc:"Evaluación detallada por criterios y niveles."},lista_cotejo_a:{title:"Lista de Cotejo Simple",icon:"✅",desc:"Cumple / No cumple (binaria)."},lista_cotejo_b:{title:"Lista Ponderada",icon:"⚖️",desc:"Cotejo con pesos específicos por criterio."},escala_estimativa:{title:"Escala Estimativa",icon:"📈",desc:"Valoración cualitativa (Siempre... Nunca)."}},c={filters:{type:"",gradeId:"",subject:"",periodId:""}};function i(t){s.instruments||(s.instruments=[]);const e=y();t.innerHTML=`
  ...
  `}function b(t){const e=a[t];return`
  ...
  `}function w(t){const e=a[t.type]||{title:"Otro",icon:"📄"},o=d().find(m=>m.id===t.courseId),p=o?`${o.gradeName} ${o.sectionName}`:"--",u=o?o.materia:t.subjectName||"--";return`
    <tr class="group hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0">
      <td class="px-8 md:px-12 py-6">
        <div class="flex items-center gap-4">
           ...
           <div>
             <div class="text-sm font-black text-slate-800">${n(t.name)}</div>
      ...
      <td class="px-6 py-6">
        <div class="text-xs font-bold text-slate-600">${n(u)}</div>
        <div class="text-[10px] font-medium text-slate-400 mt-0.5">${p}</div>
      </td>
      ...
      <td class="px-6 py-6 text-right">
        <div class="flex items-center justify-end gap-2 opacity-10 md:opacity-0 group-hover:opacity-100 transition-all">
           <button onclick="window.editInstrument('${t.id}')" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Editar">
  ...
  `}function y(){const t=c.filters;return(s.instruments||[]).filter(e=>!(t.type&&e.type!==t.type||t.gradeId&&e.courseId!==t.gradeId||t.periodId&&e.periodId!==t.periodId))}function I(t){return t.type==="rubrica_analitica"?t.maxTotal||t.maxScore||0:t.type==="lista_cotejo_a"?t.maxTotal||(t.criteria?t.criteria.length:0):t.maxScore||0}window.setInstFilter=(t,e)=>{c.filters[t]=e,i(document.getElementById("p-content"))};window.createNewInstrument=t=>{r(`Iniciando creador de ${a[t].title}...`,!1)};window.editInstrument=t=>{r(`Cargando editor para el instrumento ${t}...`,!1)};window.deleteInstrument=t=>{confirm("¿Estás seguro de que deseas eliminar este instrumento?")&&(s.instruments=s.instruments.filter(e=>e.id!==t),v(),i(document.getElementById("p-content")),r("Instrumento eliminado"))};window.openInstrumentCreator=()=>{f("m-inst-type")};window.RENDERS.instrumentos=t=>i(t);export{l as BASIC_INSTRUMENT_TYPES,a as INSTRUMENT_META,i as renderInstrumentsPanel};
