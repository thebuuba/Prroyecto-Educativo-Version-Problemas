import{S as i,A as G,g as u,b as M,a as I,f as w,B as m,c as $,d as k,r as j,s as B,e as p,h as z,i as H,p as N,j as P,k as v,l as S,m as E,u as T}from"./main-DzuOqHLs.js";function C(){return i.activityViewMode||G}window.setActView=t=>{i.activityViewMode=["blocks","matrix","config"].includes(t)?t:"blocks",v(),S("actividades")};function F(){const t=C();return`
  ...
  `}function L(t,a,s,o){const l=M(t),e=I(t),n=m[t]||"var(--blue)",r=$[t]||"📄",c=k[t]||t,x=e>0?Math.min(l/e*100,100):0;let b="";return a.length===0?b=`
    ...
    `:b=a.map((d,h)=>{const g=o.map(f=>(i.notas[f.id]||{})[d.id]).filter(f=>f!==void 0),y=g.length?j(g.reduce((f,V)=>f+V,0)/g.length):null,A=d.instrumentId;return`
        <div class="flex items-center gap-4 py-4 border-b border-slate-50 last:border-0 group/row">
          <div class="w-6 text-xs font-bold text-slate-300">${h+1}</div>
  ...
  `}function O(){const t=u(i.activeGroupId),a=B(i.activeGroupId);return`
  ...
  `}function R(){const t=B(i.activeGroupId);if(t.length===0)return`
    ...
    `;const a=u(i.activeGroupId),s={};p.forEach(e=>s[e]=a[e].activities);let o=`
  ...
  `;p.forEach(e=>{const n=s[e].length;n>0&&(o+=`
        <th colspan="${n+1}" class="p-4 text-center border-r border-slate-200" style="background: ${m[e]}10; color: ${m[e]}">
  ...
  `,p.forEach(e=>{s[e].forEach((n,r)=>{o+=`
    ...
    `,p.forEach(n=>{s[n].forEach(r=>{const c=(i.notas[e.id]||{})[r.id],x=c===0||!!c;l+=`
        ...
        `}),s[n].length>0&&(l+=`<td class="p-3 text-center bg-slate-50/30 font-bold text-slate-900 border-r border-slate-200">${z(e.id,n)}</td>`)}),l+=`<td class="p-4 text-center font-black text-blue-600 bg-blue-50/30 border-l-2 border-slate-900">${H(e.id,i.activeGroupId)??"---"}</td></tr>`}),l+="</tbody>",`
    <div class="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm animate-in fade-in duration-500">
      <div class="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
        ...
        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">${P(i.activeGroupId)}</div>
      </div>
  ...
  `}function _(t){const s=u(i.activeGroupId)[t].activities,o=M(t),l=I(t),e=j(o-l),n=m[t]||"var(--blue)",r=$[t]||"📄",c=k[t]||t,x=e===0?"text-emerald-500 bg-emerald-50":e>0?"text-rose-500 bg-rose-50":"text-amber-500 bg-amber-50",b=e===0?"Total coincide con la meta":e>0?`Sobran ${w(e)} pts`:`Faltan ${w(Math.abs(e))} pts`;return`
    <div class="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
      ...
      <div class="space-y-4 mb-8">
        ${s.map((d,h)=>`
          <div class="flex items-center gap-4 animate-in fade-in duration-300">
            <div class="w-6 text-[10px] font-black text-slate-300">${h+1}</div>
      ...
      <div class="pt-8 border-t border-slate-50">
        ...
        <div class="flex justify-between mt-3">
           <span class="text-xs font-bold text-slate-400">${w(o)} registrados</span>
           <span class="text-xs font-bold text-slate-900">${l} meta</span>
        </div>
      </div>
    </div>
  `}window.updateBlockMeta=(t,a)=>{const s=parseFloat(a)||100;u(i.activeGroupId)[t].meta=s,v(),window.go("actividades")};window.handleActNameInput=(t,a,s)=>{const o=E(a);o&&(o.activity.name=s.value),v()};window.updateActPts=(t,a,s)=>{const o=parseFloat(s)||0,l=E(a);l&&(l.activity.pts=o),v()};window.addActToBlock=t=>{const a=u(i.activeGroupId)[t].activities;a.push({id:T(),name:`Actividad ${a.length+1}`,pts:20,courseId:i.activeGroupId,periodId:i.activePeriodId,instrumentId:null,instrumentIds:[],instrumentHistory:[]}),v(),window.go("actividades")};window.removeActFromBlock=(t,a)=>{const s=u(i.activeGroupId)[t];s.activities=s.activities.filter(o=>o.id!==a),i.evaluations=i.evaluations.filter(o=>!(o.activityId===a&&(o.periodId||"P1")===i.activePeriodId)),v(),window.go("actividades")};window.autoAdjustBlock=t=>{const a=u(i.activeGroupId)[t],s=a.activities;if(!s.length)return;const o=a.meta||100,l=Math.floor(o/s.length),e=o%s.length;s.forEach((n,r)=>{n.pts=l+(r<e?1:0)}),v(),window.go("actividades")};function q(t){const a=C();t.innerHTML=`
