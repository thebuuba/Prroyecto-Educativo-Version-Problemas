import{e as p,C as m,S as d,d as h,F as k,s as y,B as c,v as u,x,G as $,q as w,H as R,w as j,k as f,z as E,I as B,J as T,r as L}from"./main-DGpyBctX.js";function g(){const o=m(d.activeGroupId)||"grupo",t=h()||"periodo",r=`${o}-${t}`;return`reporte-${k(r).replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")||"general"}`}function i(o){return o?Array.from(o.querySelectorAll("tr")).map(t=>Array.from(t.querySelectorAll("th,td")).map(r=>String(r.textContent||"").replace(/\s+/g," ").trim())).filter(t=>t.length>0):[]}function v(o){if(!o.length)return"<table><tbody><tr><td>Sin datos</td></tr></tbody></table>";const[t,...r]=o,e=`<thead><tr>${t.map(s=>`<th>${p(s)}</th>`).join("")}</tr></thead>`,l=`<tbody>${r.map(s=>`<tr>${s.map(n=>`<td>${p(n)}</td>`).join("")}</tr>`).join("")}</tbody>`;return`<table>${e}${l}</table>`}function b(o,t,r){const e=p(o),l=v(t),s=v(r);return`<!doctype html><html><head><meta charset="utf-8"><title>${e}</title>
  <style>
    ...
    <h1>${e}</h1>
    <p class="sub">${p(m(d.activeGroupId)||"Sin grupo")} • ${p(h())}</p>
    <h2>Tabla de calificaciones</h2>
    ${l}
    <h2>Resumen anual por estudiante</h2>
    ${s}
  </body></html>`}function M(){window.reportDownloadExcel=()=>{const o=document.querySelector(".reportes-stack table"),t=document.querySelector(".annual-report-table"),r=i(o),e=i(t);if(!r.length&&!e.length){typeof window.toast=="function"&&window.toast("No hay datos para exportar",!0);return}const l=b("Reporte AulaBase",r,e),s=new Blob(["\uFEFF",l],{type:"application/vnd.ms-excel;charset=utf-8"}),n=URL.createObjectURL(s),a=document.createElement("a");a.href=n,a.download=`${g()}.xls`,document.body.appendChild(a),a.click(),a.remove(),setTimeout(()=>URL.revokeObjectURL(n),1200)},window.reportDownloadPdf=async()=>{const o=document.querySelector(".reportes-stack table"),t=document.querySelector(".annual-report-table"),r=i(o),e=i(t);if(!(!r.length&&!e.length))try{const s=b("Reporte AulaBase",r,e).replace("</body>","<script>window.addEventListener('load', function () { setTimeout(function () { try { window.focus(); window.print(); } catch (_) {} }, 180); });<\/script></body>"),n=new Blob([s],{type:"text/html;charset=utf-8"}),a=URL.createObjectURL(n);if(!window.open(a,"_blank")){URL.revokeObjectURL(a),typeof window.toast=="function"&&window.toast("Habilita pop-ups para imprimir.",!0);return}setTimeout(()=>URL.revokeObjectURL(a),6e4)}catch(l){console.error("Error PDF:",l)}},window.reportDownloadWord=()=>{const o=document.querySelector(".reportes-stack table"),t=document.querySelector(".annual-report-table"),r=i(o),e=i(t);if(!r.length&&!e.length)return;const l=b("Reporte AulaBase",r,e),s=new Blob(["\uFEFF",l],{type:"application/msword"}),n=URL.createObjectURL(s),a=document.createElement("a");a.href=n,a.download=`${g()}.doc`,document.body.appendChild(a),a.click(),a.remove(),setTimeout(()=>URL.revokeObjectURL(n),1200)}}function A(){return`
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 animate-in fade-in slide-in-from-top-4 duration-500">
      ...
      <div class="flex items-center gap-3">
         <span class="px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold text-slate-500">GRUPO: ${m(d.activeGroupId)}</span>
  ...
  `}function z(o){if(o.length===0)return`
      ...
      <div class="overflow-x-auto custom-scrollbar">
        <table class="w-full border-collapse annual-report-table">
          ...
          <tbody class="divide-y divide-slate-50 text-slate-700">
            ${o.map(e=>{const l=["P1","P2","P3","P4"].map(a=>f(e.id,d.activeGroupId,a)),s=c.map(a=>B(e.id,a,d.activeGroupId)),n=T(e.id,d.activeGroupId);return`
                <tr class="hover:bg-slate-50/50 transition-colors text-sm">
                  ...
                  ${s.map(a=>`<td class="p-4 text-center font-medium">${a!==null?L(a):'<span class="opacity-30">?</span>'}</td>`).join("")}
  ...
  `}function D(o){const t=y(d.activeGroupId);o.innerHTML=`
  ...
  `}function U(){M(),window.RENDERS||(window.RENDERS={}),window.RENDERS.reportes=D}export{U as init,D as renderReportsPanel};
