 ...
 */
export function renderSectionCreatePanel(container) {
          ...
          <section class="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" id="sec-room" placeholder="Ej. Aula 102" value="${FormState.room}"
...
function updateSubjectGrid() {
  ...
  const catalog = AREA_SUBJECTS.Primaria.find(a => a.area === FormState.area);
...
function updatePreviews() {
  ...
  setHidden('sec-room', FormState.room);
}
...
window.updateSectionCreateField = (field, value) => {
  ...
  if (field === 'area') {
    ...
    // Auto-select si solo hay una
    const catalog = AREA_SUBJECTS.Primaria.find(a => a.area === value);
...
window.confirmSaveSection = async () => {
  ...
  try {
    // Inyectar valores para compatibilidad legacy que saveSec espera del DOM
    ...
    setVal('sec-room', FormState.room);

    await window.saveSec();
    // window.saveSec ya redirige a 'estudiantes' si tiene éxito
  } catch (err) {
...
window.RENDERS['section-create'] = renderSectionCreatePanel;
