import { escapeHtml } from '../../../../../../js/core/utils.ts';
...
export function renderizarScheduleContent(deps) {
  const { UI } = deps;
...
function renderizarWeeklySchedule({ S }) {
...
function renderizarScheduleCell(cell, weekday, start, end, S) {
  if (!cell) {
...
function renderizarMonthlyCalendar({ UI, getPlannerEvents, attendanceMonthStart, escapeHtml }) {
  return `
...
function renderizarUpcomingEvents({ getPlannerEvents, escapeHtml }) {
...
function renderizarCalendarDays({ UI, getPlannerEvents, attendanceMonthStart }) {
