/**
 * Renderizado del panel de planificaciones.
 * Mantiene toda la composición visual separada de las acciones y mutaciones de estado.
 */
...
import { LESSON_PLAN_TRANSVERSAL_AXES } from '../../../../../../js/core/constants.ts';
...
function renderizarDraftList(drafts) {
  if (!drafts.length) {
    return `
...
function renderizarPlanCard(plan) {
  return `
...
function renderizarStepRail(activeId) {
  const steps = [
...
function renderizarEditorGeneral(draft) {
...
function renderizarEditorCurriculum(draft) {
  return `
...
function renderizarEditorDesign(draft) {
  return `
    <div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
...
function renderizarEditorEvaluation(draft) {
  return `
    <div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
...
function renderizarEditorPreview(draft) {
  return `
    <div class="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div class="flex items-center justify-between">
...
export function renderizarPlanningPanel(container) {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div class="lg:col-span-8 flex flex-col gap-8">
            ...
            <div class="space-y-6">
              ...
              ${plans.length ? `
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ${plans.map(renderizarPlanCard).join('')}
                </div>
              ` : `
