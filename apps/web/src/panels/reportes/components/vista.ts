...
import { BNAME, BICON, BCOLOR } from '../../../../../../js/core/config.ts';
import { BLOCKS } from '../../../../../../js/core/constants.ts';
import { 
  fmtNum, 
  getGroupLabel, 
  periodName, 
 ...
 * @returns {string} HTML.
 */
function renderizarReportHeader() {
  return `
 ...
 * @returns {string} HTML.
 */
function renderizarBlockAverages() {
 ...
 * @returns {string} HTML.
 */
function renderizarGradesAndAnnual(ests) {
  if (ests.length === 0) {
    return `
 ...
 */
export function renderizarReportsPanel(container) {
