...
import { 
  ...
  BICON, 
  BCOLOR 
} from '../../../../../../js/core/config.ts';
import { 
  BLOCKS, 
  ACT_VIEW_MODE_DEFAULT 
} from '../../../../../../js/core/constants.ts';
import { 
  fmtNum, 
  round2, 
  uid,
 ...
 */
export function obtenerActViewMode() {
  ...
  return `
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
      ...
      <div class="flex p-1 bg-slate-100 rounded-2xl shrink-0">
        <button data-activity-action="change-matrix-view" data-matrix-view="blocks" class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${mode === 'blocks' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}">
          Bloques
        </button>
        <button data-activity-action="change-matrix-view" data-matrix-view="matrix" class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${mode === 'matrix' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}">
          Matriz
        </button>
        <button data-activity-action="change-matrix-view" data-matrix-view="config" class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${mode === 'config' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}">
          Configuración
        </button>
      </div>
    </header>
 ...
 */
function renderizarBlockCard(b, acts, cfg, ests) {
      ...
      return `
        <div class="flex items-center gap-4 py-4 border-b border-slate-50 last:border-0 group/row">
          ...
          <div class="flex items-center gap-2">
            <button data-activity-action="evaluate-student" data-activity-id="${escapeHtml(a.id)}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Evaluar">
  ...
  ests.forEach(e => {
    ...
    BLOCKS.forEach(b => {
      blockActs[b].forEach(a => {
        ...
        bodyHtml += `
          <td class="p-3 text-center cursor-pointer hover:bg-blue-50 transition-colors" data-activity-action="evaluate-student" data-activity-id="${escapeHtml(a.id)}" data-student-id="${escapeHtml(e.id)}">
 ...
 */
export function renderizarActivitiesConfigBlock(b) {
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <div class="flex items-center gap-2 mt-0.5">
               ...
               <input type="number" value="${meta}" data-activity-action="select-block" data-activity-event="change" data-block-id="${escapeHtml(b)}" class="w-12 text-[10px] font-black text-blue-600 bg-blue-50 border-none rounded-md px-1 py-0.5 text-center focus:ring-2 focus:ring-blue-500">
            </div>
          </div>
        </div>
        <div class="flex items-center gap-2">
           <button data-activity-action="create" data-block-id="${escapeHtml(b)}" class="px-4 py-2 bg-slate-100 text-slate-800 rounded-xl text-xs font-bold hover:bg-slate-200 transition-colors">
             + Actividad
           </button>
           <button data-activity-action="calculate-average" data-block-id="${escapeHtml(b)}" class="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-xs font-bold hover:bg-blue-100 transition-colors">
             Autoajustar
           </button>
        </div>
      </div>
      ...
      <div class="space-y-4 mb-8">
        ${acts.map((a, i) => `
          <div class="flex items-center gap-4 animate-in fade-in duration-300">
            ...
            <input type="text" value="${a.name}" data-activity-action="edit" data-activity-event="input" data-block-id="${escapeHtml(b)}" data-activity-id="${escapeHtml(a.id)}" placeholder="Nombre actividad..." class="flex-1 px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all outline-none">
            <div class="flex items-center gap-2 w-[100px]">
              <input type="number" value="${a.pts}" data-activity-action="change-grade" data-activity-event="input" data-block-id="${escapeHtml(b)}" data-activity-id="${escapeHtml(a.id)}" class="w-full px-3 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm font-bold text-center focus:ring-2 focus:ring-blue-500 transition-all outline-none">
              <span class="text-[10px] font-bold text-slate-400">pts</span>
            </div>
            <button data-activity-action="delete" data-block-id="${escapeHtml(b)}" data-activity-id="${escapeHtml(a.id)}" class="p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
